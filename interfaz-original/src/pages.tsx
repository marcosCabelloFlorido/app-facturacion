            ...
            rows.map((d, index) => (
              ...
              >
                ...
                <td className="doc-status">
                  {kind === 'sales' && d.kind === 'invoice' ? (
                    <InvoiceStatusPipeline doc={d} status={rowStatuses[index]} />
                  ) : (
...
export function Catalog({ notify, readonly }: Props) {
  ...
  return (
    <>
      ...
      <PageHeading
        title={selectedProduct?.name || 'Catálogo'}
        menu={
          selectedProduct
            ? [
                !readonly && {
                  ...
                  group: 'Artículo',
                  icon: Settings2,
                  onAction: () => setEditing(selectedProduct),
                },
                !readonly && {
                ...
                !readonly &&
                  selectedProduct.active && {
                    ...
                    href:
                      ...
                      selectedProduct.id +
                      '&from=' +
                      encodeURIComponent(catalogRoute + '&product=' + selectedProduct.id),
                  },
                !readonly &&
                  selectedProduct.active && {
        ...
        status={
          selectedProduct && (
            <DocumentStatusLabel status={selectedProduct.active ? 'Activo' : 'Archivado'} />
          )
        }
        backLink={
          selectedProductId
            ? { href: '#' + catalogRoute, label: 'Catálogo' }
            : showList
      ...
      <div className={`page-content${showList ? ' catalog-list-page' : ' catalog-summary-page'}`}>
        ...
        {selectedProductId &&
          (loading ? (
            ...
            <section className="panel">
              <PanelHeading title="Datos del artículo" />
              <dl className="catalog-record-data">
                <div>
                  <dt>Código</dt>
                  <dd>{selectedProduct.sku || 'Sin código'}</dd>
                </div>
                <div>
                  <dt>Descripción</dt>
                  <dd>{selectedProduct.description || 'Sin descripción'}</dd>
                </div>
                <div>
                  <dt>Unidad</dt>
                ...
                <div>
                  ...
                  <dd>{euros(selectedProduct.unitPrice)}</dd>
                </div>
                <div>
                  <dt>IVA</dt>
                  <dd>{selectedProduct.taxRate} %</dd>
                </div>
                {selectedProduct.exemptionReason && (
                  <div>
                    <dt>Motivo de exención</dt>
                    <dd>{selectedProduct.exemptionReason}</dd>
                  </div>
                )}
              </dl>
            </section>
